...
SetMissionDvar( "lastplayedlevel", 4 );
thread overalltimer();
...
thread segmenttimer1( 10 );
thread waitTrigger01();
wait 4;
...
{
thread trigger01_b();
trig01 = ( 2684, 24590, 31 );
while( true )
{
if( distance( level.player.origin, trig01 ) < 100 )
{
level notify( "trigger01_hit" );
}
wait 0.1;
}
}
trigger01_b()
{
trig01_b = ( 1669, 25796, 268 );
while( true )
{
if( distance( level.player.origin, trig01_b ) < 120 )
{
level notify( "trigger01_hit" );
}
wait 0.1;
...
trigger02()
{
trig02 = ( 3246, 23174, 204 );
while( true )
{
if( distance( level.player.origin, trig02 ) < 100 )
{
level notify( "trigger02_hit" );
}
wait 0.1;
}
}
...
{
thread trigger01();
level waittill( "trigger01_hit" );
thread segmenttimer2( 11 );
thread waitTrigger02();
}
waitTrigger01_b()
{
thread trigger01();
...
{
thread trigger02();
SetMissionDvar( "lastplayedsection", 11 );
level waittill( "trigger02_hit" );
level notify( "segment2_stop" );
sectionUpdatePBLastLevelSeg();
addtoseconds = GetDvarFloat( "timervalue" );
addtoseconds += 2.4;
SetMissionDvar( "timervalue", addtoseconds );
addtoseconds = GetDvarFloat( "fulltimervalue" );
addtoseconds += 2.4;
SetMissionDvar( "fulltimervalue", addtoseconds );
}